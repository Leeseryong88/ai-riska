...
import { Card, Button, Input } from '../ui/Button';
import { QRCodeSVG } from 'qrcode.react';
import { Download, Printer, Loader2, Save, Edit2, ExternalLink } from 'lucide-react';
...
export const QRCodesTab: React.FC = () => {
    ...
    const fetchData = async (uid: string) => {
      try {
        ...
        if (adminDoc.exists()) {
          ...
          setAdminData(data);
          setTempQrText(data.qrText || '본 사업장은 안전작업 허가제 시행 구역입니다.\n작업 전 반드시 QR코드를 스캔하여 허가서를 제출하고 승인을 받은 후 작업을 시작해 주시기 바랍니다.');
          setTempQrTitle(data.qrTitle || '접근 및 작업 허가 QR');
        }
  ...
  const printQR = (id: string, name: string) => {
    ...
    const qrTitle = adminData?.qrTitle || '접근 및 작업 허가 QR';
    const qrText = adminData?.qrText || '본 사업장은 안전작업 허가제 시행 구역입니다.\n작업 전 반드시 QR코드를 스캔하여 허가서를 제출하고 승인을 받은 후 작업을 시작해 주시기 바랍니다.';
  ...
  const qrUrl = `${baseUrl}/work-permit/v/${adminId}`;
    ...
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500">사업장에 비치할 QR코드를 생성하고 출력합니다.</p>
        </div>
      </div>
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        ...
        <Card className="p-10 flex flex-col items-center text-center border-2 border-blue-100 bg-blue-50/30 max-w-md w-full h-fit">
            ...
            {isEditing ? (
              ...
              <div className="space-y-6">
                <div className="group relative inline-block">
                  <h3 className="font-bold text-xl text-gray-900">{adminData?.qrTitle || '접근 및 작업 허가 QR'}</h3>
                  <button 
                ...
                <p className="text-sm text-gray-500 whitespace-pre-wrap leading-relaxed">
                  {adminData?.qrText || '본 사업장은 안전작업 허가제 시행 구역입니다.\n작업 전 반드시 QR코드를 스캔하여 허가서를 제출하고 승인을 받은 후 작업을 시작해 주시기 바랍니다.'}
                </p>
              </div>
            )}
          </div>
          ...
          <div className="flex gap-3 w-full mt-6">
            <Button variant="outline" className="flex-1 gap-2 h-12" onClick={() => downloadQR('main', adminData?.qrTitle || '접근_QR')}>
              <Download className="w-4 h-4" /> 이미지 저장
            </Button>
            <Button variant="outline" className="flex-1 gap-2 h-12" onClick={() => printQR('main', adminData?.qrTitle || '접근 및 작업 허가')}>
        ...
        <Card className="flex-1 p-8">
          ...
          <div className="space-y-4 text-sm text-gray-600 leading-relaxed">
            ...
            <div className="p-4 bg-gray-50 rounded-xl border border-gray-100">
              ...
              <p>출력된 QR코드를 입구나 안내 데스크 등 작업자의 눈에 잘 띄는 곳에 비치해 주세요.</p>
            </div>
          </div>

...
import { Plus, FileText, LayoutGrid, Edit, Loader2 } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import { useAuth } from '@/app/context/AuthContext';
import { db } from '@/app/lib/firebase';
import { collection, addDoc, updateDoc, deleteDoc, doc, query, where, orderBy, getDocs, limit, startAfter, serverTimestamp, QueryDocumentSnapshot, DocumentData } from 'firebase/firestore';
import { uploadBase64 } from '@/app/work-permit/_lib/storage';

const LOGS_PER_PAGE = 10;

function SafetyLogContent() {
  const { user } = useAuth();
  ...
  const handleCreateLog = async (data: SafetyLog) => {
    ...
    try {
      ...
      // 2. Firestore 저장 (undefined 값 제거 및 null 처리)
  ...
  const handleUpdateLog = async (data: SafetyLog) => {
    ...
    try {
      ...
      // 2. Firestore 업데이트 (undefined 값 제거)
      const cleanData = JSON.parse(JSON.stringify(data));
        ...
        <AnimatePresence mode="wait">
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <Button className="gap-2" onClick={() => setView('create')}>
                  <Plus className="w-4 h-4" /> 일지 작성하기
                </Button>
              </div>
              ...
              {isLoading ? (
                <div className="flex flex-col items-center justify-center py-20">
                  ...
                  <p className="mt-4 text-sm text-gray-400 font-bold">일지를 불러오는 중...</p>
                </div>
              ) : (
                <div className="space-y-6">
                  <LogList logs={logs} onSelect={(log) => { setSelectedLog(log); setView('detail'); }} onDelete={handleDeleteLog} />
            ...
            >
              <div className="flex items-center gap-4 mb-8">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-500 font-medium">오늘의 현장 안전 정보를 기록하세요.</p>
                </div>
              </div>
              <LogForm 
          ...
          {view === 'detail' && selectedLog && (
            ...
            >
              <LogDetail
                log={selectedLog}
                onEdit={(log) => { setSelectedLog(log); setView('edit'); }}
            ...
            >
              <div className="flex items-center gap-4 mb-8">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-500 font-medium">{selectedLog.date} 일지 내용을 수정합니다.</p>
                </div>
              </div>
              <LogForm

...
import { Trash2, ExternalLink, Calendar, Info, Calculator, Printer, Edit2, Check, X } from 'lucide-react';
...
export function ManagementFeePlanTab() {
  const { user } = useAuth();
  const [items, setItems] = useState<MFPRecord[]>([]);
  const [loading, setLoading] = useState(true);
  const [listPage, setListPage] = useState(1);
  const [selectedItem, setSelectedItem] = useState<MFPRecord | null>(null);
  ...
  const fetchItems = async () => {
    ...
    setLoading(true);
    try {
      const aggregated: MFPRecord[] = [];
      let cursor: QueryDocumentSnapshot<DocumentData> | undefined;
      while (true) {
        let q = query(
          collection(db, 'managementFeePlans'),
        ...
        aggregated.push(
          ...querySnapshot.docs.map((d) => ({ id: d.id, ...d.data() }) as MFPRecord)
        );
      ...
      console.error('Error fetching management fee plans:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id: string, e: React.MouseEvent) => {
    ...
    try {
      await deleteDoc(doc(db, 'managementFeePlans', id));
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const docRef = doc(db, 'managementFeePlans', selectedItem.id);
      ...
      console.error('Error updating management fee plan:', error);
    ...
    <div className="space-y-6">
      {items.length === 0 ? (
        <div className="text-center py-20 bg-gray-50 rounded-2xl border-2 border-dashed border-gray-200">
          ...
          <p className="text-gray-500 font-bold">저장된 관리비 계획서가 없습니다.</p>
        </div>
      ) : (
        <>
            ...
            {pagedItems.map((item, index) => (
              ...
              >
                <div className="hidden md:flex md:col-span-1 items-center justify-center">
                  <span className="text-sm font-black text-amber-600">{(listPage - 1) * LIST_PAGE_SIZE + index + 1}</span>
                </div>
                <div className="col-span-1 md:col-span-7 flex items-center gap-3">
                  <div className="md:hidden w-8 h-8 shrink-0 rounded-lg bg-amber-100 flex items-center justify-center">
                    <span className="text-xs font-black text-amber-600">{(listPage - 1) * LIST_PAGE_SIZE + index + 1}</span>
                  </div>
                  <div className="hidden md:flex w-10 h-10 rounded-lg bg-orange-100 items-center justify-center shrink-0">
                ...
                {isEditing ? (
                  <div className="h-full flex flex-col gap-4">
                    ...
                    <div
                      ...
                      className="flex-1 w-full p-8 bg-white border-2 border-blue-100 rounded-2xl focus:ring-2 focus:ring-blue-500 outline-none overflow-auto mfp-container"
                  ...
                  <div 
                    className="bg-white p-8 border border-gray-100 rounded-2xl overflow-auto mfp-container"
